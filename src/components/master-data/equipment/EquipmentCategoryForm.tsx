...
import type { EquipmentCategoryMaster } from '../../../types/equipment-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: EquipmentCategoryMaster | null;
  onSave: (data: Partial<EquipmentCategoryMaster>) => void;
}

export const EquipmentCategoryForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<EquipmentCategoryMaster>>({
    code: '',
    name: '',
    description: '',
    equipmentTypeId: '',
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[16px] font-bold text-on-surface">
              {initialData ? 'Editar Categoria' : 'Nova Categoria de Equipamento'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              required
              placeholder="Ex: Tratores Pesados 4x4"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Descrição das especificações e aplicação operacional..."
