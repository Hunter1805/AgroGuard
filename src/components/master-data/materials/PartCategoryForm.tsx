...
import type { PartCategoryMaster } from '../../../types/material-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: PartCategoryMaster | null;
  onSave: (data: Partial<PartCategoryMaster>) => void;
}

export const PartCategoryForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<PartCategoryMaster>>({
    code: '',
    name: '',
    description: '',
    parentCategoryId: '',
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        code: `PEC-${Math.floor(100 + Math.random() * 900)}`,
        name: '',
        description: '',
        parentCategoryId: '',
        status: 'ativo',
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[16px] font-bold text-on-surface">
              {initialData ? 'Editar Categoria de Peça' : 'Nova Categoria de Peça'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              required
              placeholder="Ex: Filtros, Lubrificantes, Correias, Sistema Elétrico"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
