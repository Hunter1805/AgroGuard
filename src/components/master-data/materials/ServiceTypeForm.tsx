...
import type { ServiceTypeMaster } from '../../../types/material-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: ServiceTypeMaster | null;
  onSave: (data: Partial<ServiceTypeMaster>) => void;
}

export const ServiceTypeForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<ServiceTypeMaster>>({
    code: '',
    name: '',
    description: '',
    status: 'ativo',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-surface-container-lowest border border-white/10 rounded-xl p-5 shadow-2xl space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Nome do Tipo de Serviço</label>
            <input
              type="text"
              value={formData.name || ''}
              onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
              required
              placeholder="Ex: Mecânica Diesel, Torno e Solda, Calibração de Pulverizador"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
