...
import type { Sector } from '../../../types/organization-master-data';
...
export const SectorForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        code: `SEC-${Math.floor(100 + Math.random() * 900)}`,
        name: '',
        description: '',
        companyId: 'EMP-01',
        responsibleName: '',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-surface-container-lowest border border-white/10 rounded-xl p-5 shadow-2xl space-y-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[16px] font-bold text-on-surface">
              {initialData ? 'Editar Setor' : 'Novo Setor Operacional'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Nome do Setor</label>
            <input
              type="text"
              value={formData.name || ''}
              onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
              required
              placeholder="Ex: Setor de Preparo de Solo"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Nome do supervisor ou gestor"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Detalhamento das atividades e escopo do setor..."
