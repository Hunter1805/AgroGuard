...
import type { CostCenter } from '../../../types/organization-master-data';
...
export const CostCenterForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-surface-container-lowest border border-white/10 rounded-xl p-5 shadow-2xl space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Código Único</label>
            <input
              type="text"
              value={formData.code || ''}
          ...
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Nome do Centro de Custo</label>
            <input
              type="text"
              value={formData.name || ''}
              onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
              required
              placeholder="Ex: Manutenção da Frota de Tratores"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>

          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Gestor Responsável</label>
            <input
              type="text"
              value={formData.managerName || ''}
              onChange={(e) => setFormData((p) => ({ ...p, managerName: e.target.value }))}
              placeholder="Nome do gestor financeiro/operacional"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>

          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Observações</label>
            <textarea
          ...
          <div className="flex items-center gap-2 pt-1">
            <input
              ...
              onChange={(e) => setFormData((p) => ({ ...p, isClosed: e.target.checked }))}
              className="rounded border-white/20 bg-surface-container-highest text-primary focus:ring-primary/40"
            />
            <label htmlFor="isClosed" className="text-[12px] text-on-surface cursor-pointer select-none">
              Encerrar centro de custo (impede novos lançamentos)
            </label>
          </div>
