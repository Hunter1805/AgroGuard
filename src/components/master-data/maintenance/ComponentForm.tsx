...
import type { ComponentMaster } from '../../../types/maintenance-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: ComponentMaster | null;
  onSave: (data: Partial<ComponentMaster>) => void;
}

export const ComponentForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<ComponentMaster>>({
    code: '',
    name: '',
    description: '',
    systemId: 'SIS-01',
      ...
      <div className="relative z-10 w-full max-w-md bg-surface-container-lowest border border-white/10 rounded-xl p-5 shadow-2xl space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Part Number Fabricante</label>
              <input
                type="text"
                value={formData.manufacturerPartNumber || ''}
                onChange={(e) => setFormData((p) => ({ ...p, manufacturerPartNumber: e.target.value }))}
                placeholder="Ex: PN-98700"
          ...
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Nome do Componente</label>
            <input
              type="text"
              value={formData.name || ''}
              onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
              required
              placeholder="Ex: Bomba de Óleo Principal, Filtro Racor"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
