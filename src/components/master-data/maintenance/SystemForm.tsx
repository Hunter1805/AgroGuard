...
import type { SystemMaster } from '../../../types/maintenance-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: SystemMaster | null;
  onSave: (data: Partial<SystemMaster>) => void;
}

export const SystemForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<SystemMaster>>({
    code: '',
    name: '',
    description: '',
    status: 'ativo',
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        code: `SIS-${Math.floor(100 + Math.random() * 900)}`,
        name: '',
        description: '',
        status: 'ativo',
      });
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[16px] font-bold text-on-surface">
              {initialData ? 'Editar Sistema Técnico' : 'Novo Sistema Técnico'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              required
              placeholder="Ex: Motor, Transmissão, Sistema Hidráulico"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Detalhamento das características técnicas do sistema..."
