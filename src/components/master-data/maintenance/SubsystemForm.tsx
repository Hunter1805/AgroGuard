...
import type { SubsystemMaster } from '../../../types/maintenance-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: SubsystemMaster | null;
  onSave: (data: Partial<SubsystemMaster>) => void;
}

export const SubsystemForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<SubsystemMaster>>({
    code: '',
    name: '',
    description: '',
    systemId: 'SIS-01',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-surface-container-lowest border border-white/10 rounded-xl p-5 shadow-2xl space-y-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[16px] font-bold text-on-surface">
              {initialData ? 'Editar Subsistema' : 'Novo Subsistema Técnico'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Nome do Subsistema</label>
            <input
              type="text"
              value={formData.name || ''}
              onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
              required
              placeholder="Ex: Lubrificação, Arrefecimento, Injeção Eletrônica"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
