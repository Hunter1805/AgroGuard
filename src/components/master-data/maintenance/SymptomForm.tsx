...
import type { SymptomMaster } from '../../../types/maintenance-master-data';

interface FormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: SymptomMaster | null;
  onSave: (data: Partial<SymptomMaster>) => void;
}

export const SymptomForm: React.FC<FormProps> = ({ isOpen, onClose, initialData, onSave }) => {
  const [formData, setFormData] = useState<Partial<SymptomMaster>>({
    code: '',
    name: '',
    description: '',
    status: 'ativo',
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        code: `SIM-${Math.floor(100 + Math.random() * 900)}`,
        name: '',
        description: '',
        status: 'ativo',
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-surface-container-lowest border border-white/10 rounded-xl p-5 shadow-2xl space-y-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[16px] font-bold text-on-surface">
              {initialData ? 'Editar Sintoma' : 'Novo Sintoma de Falha'}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-medium text-on-surface-variant uppercase mb-1">Nome do Sintoma</label>
            <input
              type="text"
              value={formData.name || ''}
              onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
              required
              placeholder="Ex: Ruído anômalo no motor, fumaça preta, vibração excessiva"
              className="w-full bg-surface-container-highest border border-white/10 rounded-lg p-2 text-[13px] text-on-surface focus:outline-none focus:border-primary/50"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Detalhamento das características do sintoma relatado pelo operador..."
