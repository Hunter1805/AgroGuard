...
export const TiresReportView: React.FC = () => {
  const { filters, updateFilters, resetFilters } = useReportFilters();
  const { data, loading } = useReportData('pneus', filters);
  ...
  return (
    <div className="space-y-4 text-xs animate-fade-in">
      <ReportHeader
        ...
        subtitle="Sulcos críticos, pressões fora do padrão, status e patrimônio acumulado"
        columns={colsToDisplay}
        onToggleColumn={handleToggleColumn}
        onExport={fmt => data && exportData('Relatório de Pneus', 'pneus', fmt, { ...data, columns: colsToDisplay })}
      />
      ...
      <div className="glass-card rounded-2xl border border-white/10 overflow-hidden">
        ...
        {loading ? (
          <div className="p-12 text-center text-xs text-on-surface-variant">Carregando relatório de pneus...</div>
